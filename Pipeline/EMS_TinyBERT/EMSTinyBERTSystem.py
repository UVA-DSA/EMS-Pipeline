...
import torch
import os
from EMS_TinyBERT.default_sets import seed_everything, device, ungroup_p_node
import numpy as np
import warnings
import yaml
from EMS_TinyBERT.utils import AttrDict, onehot2p, convert_label, preprocess
from EMS_TinyBERT.Heterogeneous_graph import HeteroGraph
from EMS_TinyBERT.model import EMSMultiModel
...
class EMSTinyBERT(nn.Module):
    def __init__(self, config, date):
        ...
        self.save_model_root = os.path.join('EMS_TinyBERT/models', '{}'.format(self.clean_model_date))
        if self.config.graph == 'hetero':
            signs_df = pd.read_excel('./EMS_TinyBERT/config_file/All Protocols Mapping.xlsx')
            impre_df = pd.read_excel('./EMS_TinyBERT/config_file/Impression Protocol.xlsx')
            med_df = pd.read_excel('./EMS_TinyBERT/config_file/Medication Protocol.xlsx')
            proc_df = pd.read_excel('./EMS_TinyBERT/config_file/Procedure Protocol.xlsx')
...
def EMSTinyBERTSystem(Window, EMSTinyBERTQueue, ProtocolQueue):
    ...
    if Window:
        ...
        ProtocolSignal.signal.connect(Window.UpdateProtocolBox)
        
    # initialize
    seed_everything(3407)
    from EMS_TinyBERT.default_sets import model_name
