...
class MainWindow(QWidget):
    ...
    def StartButtonClick(self):
        ...
        if(self.ComboBox.currentText() == 'Microphone'):
            if(self.GoogleSpeechRadioButton.isChecked()):
                ...
                self.SpeechThread = StoppableThread(
                    target=WhisperMicStream.Whisper, args=(self, TranscriptQueue, EMSAgentSpeechToNLPQueue))

            # elif(self.DeepSpeechRadioButton.isChecked()):
            #     self.SpeechThread = StoppableThread(
            #         target=DeepSpeechMicStream.DeepSpeech, args=(self, TranscriptQueue,))
            
            
            elif(self.MLSpeechRadioButton.isChecked()):
                self.SpeechThread = StoppableThread(
                    target=WavVecMicStream.WavVec, args=(self, TranscriptQueue,))
        ...
        # If Other Audio File
            ...
            if(self.GoogleSpeechRadioButton.isChecked()):
                self.SpeechThread = StoppableThread(target=GoogleSpeechFileStream.GoogleSpeech, args=(self, TranscriptQueue, EMSAgentSpeechToNLPQueue, str(audio_fname), data_path, audiostream, transcriptStream,))

            
            elif(self.WhisperRadioButton.isChecked()):
                self.SpeechThread = StoppableThread(
                    target=WhisperFileStream.Whisper, args=(self, TranscriptQueue, EMSAgentSpeechToNLPQueue, str(audio_fname),))

            elif(self.DeepSpeechRadioButton.isChecked()):
                self.SpeechThread = StoppableThread(target=DeepSpeechFileStream.DeepSpeech, args=(
                    self, TranscriptQueue, str(audio_fname),))

                    
            elif(self.MLSpeechRadioButton.isChecked()):
                self.SpeechThread = StoppableThread(target=WavVecFileStream.WavVec, args=(self, TranscriptQueue, str(audio_fname),)) #(target=DeepSpeechFileStream.DeepSpeech, args=(self, TranscriptQueue, str(audio_fname),))
        ...
        # If Text File
            ...
            self.SpeechThread = StoppableThread(target=TextSpeechStream.TextSpeech, args=(
                self, TranscriptQueue, str(text_fname),))
            self.SpeechThread.start()
        ...
        # If a Hard-coded Audio test file
            ...
            if(self.GoogleSpeechRadioButton.isChecked()):
                ...
                self.SpeechThread = StoppableThread(
                    target=WhisperFileStream.Whisper, args=(self, TranscriptQueue, EMSAgentSpeechToNLPQueue, './Audio_Scenarios/2019_Test/' + str(self.ComboBox.currentText()) + '.wav',))

            elif(self.DeepSpeechRadioButton.isChecked()):
                self.SpeechThread = StoppableThread(target=DeepSpeechFileStream.DeepSpeech, args=(
                    self, TranscriptQueue, './Audio_Scenarios/2019_Test/' + str(self.ComboBox.currentText()) + '.wav',))

            elif(self.MLSpeechRadioButton.isChecked()):
                self.SpeechThread = StoppableThread(target=WavVecFileStream.WavVec, args=(
                    self, TranscriptQueue, './Audio_Scenarios/2019_Test/' + str(self.ComboBox.currentText()) + '.wav',)) #(target=DeepSpeechFileStream.DeepSpeech, args=( self, TranscriptQueue, './Audio_Scenarios/2019_Test/' + str(self.ComboBox.currentText()) + '.wav',))
        ...
        if(self.CognitiveSystemThread == None):
            ...
            self.CognitiveSystemThread = StoppableThread(
                target=CognitiveSystem.CognitiveSystem, args=(self, TranscriptQueue, ProtocolQueue, data_path, conceptExtractionStream, interventionStream,))
        ...
        if(self.EMSAgentThread == None):
            ...
            self.EMSAgentThread = StoppableThread(
                target=EMSAgentSystem.EMSAgentSystem, args=(self, EMSAgentSpeechToNLPQueue, ProtocolQueue))
        ...
        if(self.FeedbackThread == None):
            ...
            self.FeedbackThread = StoppableThread(
                target=Feedback.Feedback, args=(self, data_path, ProtocolQueue))
    ...
    #@pyqtSlot
    def UpdateTranscriptBox(self, input):
        item = input[0]
        text = ""
