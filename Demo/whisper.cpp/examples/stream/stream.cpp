...
//    std::this_thread::sleep_for(std::chrono::milliseconds(5000));
    ...
    while (is_running) {
        ...
        if (!use_vad) {
            ...
            const auto t_inf_start = std::chrono::high_resolution_clock::now();
        ...
        // run the inference
        {
            ...
            // print result;
            {
                ...
                for (int i = 0; i < n_segments; ++i) {
                    ...
                    const auto t_inf_end = std::chrono::high_resolution_clock::now();
                    const auto t_inf_diff = std::chrono::duration_cast<std::chrono::milliseconds>(t_inf_end - t_inf_start).count();
                    printf("Whisper Latency: %d\n",t_inf_diff);
                    ...
                    if (params.no_timestamps) {
                        printf("%s", ctext_score);

                        fd = ::open(myfifo, O_WRONLY | O_NONBLOCK);
                        ::write(fd, ctext_score, strlen(ctext_score) + 1);
                        close(fd);
