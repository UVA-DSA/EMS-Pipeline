...
const char * const * protocol_intervention_mapper(const char * protocol) {
    ...
    // Iterate through the array to check if the string exists
    for (int i = 0; i < sizeof(protocols) / sizeof(protocols[0]); i++) {

        if (strcmp(protocols[i], protocol) == 0) {
            found = true;
            break;
        }
    }
    ...
    std::cout << "Length of the string protocol: " << length << std::endl;

    printf("\n Protocol Received: %s\n", protocol);
    std::cout << "Is Protocol Found:" << std::boolalpha << found << std::endl;

    std::string ca_protocol1 = "medical - chest pain - cardiac suspected (protocol 2 - 1)";
    std::string ca_protocol2 = "general - cardiac arrest (protocol 2 - 7)";
    std::string ca_protocol3 = "general - cardiac arrest (protocol 8 - 2)";
    std::string ca_protocol4 = "medical - supraventricular tachycardia (including atrial fibrillation) medical - tachycardia medical - ventricular tachycardia with a pulse (protocol 2 - 8)";

    if (found) {

        if (std::strcmp(protocol, ca_protocol1.c_str()) == 0 || std::strcmp(protocol, ca_protocol2.c_str()) == 0 ||
            std::strcmp(protocol, ca_protocol3.c_str()) == 0 || std::strcmp(protocol, ca_protocol4.c_str()) == 0)
            return ca_interventions;
        else
            return resp_interventions;
    }

    return nullptr;
}
// Function to generate a timestamp as a string
std::string getCurrentTimestamp() {
    std::time_t now = std::time(nullptr);
    char timestamp[20]; // Sufficient to hold "YYYYMMDD_HHMMSS"
    std::strftime(timestamp, sizeof(timestamp), "%Y%m%d_%H%M%S", std::localtime(&now));
    return timestamp;
}

bool clip_image_load_from_video(const char * filename, struct clip_ctx * ctx, const int n_threads, const char ** labels,
                                size_t n_labels, float * scores, int * indices, int trial) {
    // cv::VideoCapture videoCapture(filename);

    // int fd;
    // const char * myfifo = "/tmp/protocolfifo";
    ...
    char success_protocol[2048]  = {0};
    
    bool is_success_protocol = false;

    auto startTime = std::chrono::steady_clock::now();

    std::chrono::seconds interval(3);

    const std::string myfifo = "/tmp/protocolfifo";

    // std::ifstream fifo(myfifo);

    // if(!fifo.is_open()){
    //     std::cerr << "Error: Could not open the PIPE." << std::endl;
    // }

   int fd = open(myfifo.c_str(), O_RDONLY | O_NONBLOCK);
    if (fd == -1) {
        std::cerr << "Failed to open the FIFO" << std::endl;
        return 1;
    }
    ...
    std::string csvfilename = "trial_" + std::to_string(trial) + "_" + timestamp + ".csv";
    ...
    while (true) {
        ...
        //     std::string response;
        //     std::getline(fifo,response);

        //     if(fifo.eof()){
        //         std::cerr << "Error: FIFO closed." << std::endl;

        //     }


        //     std::cout << "FIFO Received " << response << std::endl;
            

        FD_ZERO(&rfds);
        FD_SET(fd, &rfds);

        tv.tv_sec = 0;  // Set the timeout (1 second)
        tv.tv_usec = 10000;
        ...
        if (retval == -1) {
            ...
            if (FD_ISSET(fd, &rfds)) {
                ...
                if (bytesRead == -1) {
                    ...
                    std::string response(buffer, bytesRead-1);
                    // Process the response as needed
                    std::cout << "Received: " << response << "bytes read: " << bytesRead << std::endl;
                    protocol_msg = response;
                    fifo_success = true;
                    wait = false;
                }
            }
        ...
        if(wait){
            std::cout << "Waiting for first protocol signal" << std::endl;
            continue;
            }
        ...
        std::cout << "[CLIP CPP] Processing frame: " << frameIndex << std::endl;


        if(fifo_success){

        const char* protocol = protocol_msg.c_str();

        if(true){

            clip_image_u8 * img = new clip_image_u8; // Allocate memory for img

            const char * const * result;    

            if(is_success_protocol){
            result = protocol_intervention_mapper(success_protocol);
            }else{
            result = protocol_intervention_mapper(protocol);
            }
            ...
            if (result != nullptr) {
                ...
                if(not is_success_protocol)
                    strcpy(success_protocol,protocol);
                    is_success_protocol = true;


                auto start_t = std::chrono::high_resolution_clock::now(); // start timing

                for (int i = 0; result[i] != "END"; i++) {
                    std::cout << "Element " << i << ": " << result[i] << std::endl;
                    num_labels++;
                }

                float sorted_scores[num_labels];
                int sorted_indices[num_labels];
                // //zero shot label image

                // Create a copy of the data
                const char ** labels_final = new const char *[num_labels];
                for (int i = 0; i < num_labels; i++) {
                    labels_final[i] = result[i];
                }

                for (int i = 0; i < num_labels; i++) {
                    std::cout << "UPDATED labels " <<  labels_final[i] << " num_labels:" << num_labels << " scores:" << sorted_scores[i] << " indices:" << sorted_indices[i] << std::endl;
                }


                for (int i = 0; i < n_labels; i++) {
                    std::cout << "ORIGINAL labels " <<  labels[i] << " n_lables:" << n_labels << " scores:" << scores[i] << " indices:" << indices[i] << std::endl;
                }

                // clip_zero_shot_label_image(ctx, n_threads, img, labels, n_labels, scores, indices);
                ...
                frameWidth = frameWidth*scale_percent/100;
                frameHeight = frameHeight*scale_percent/100;

                cv::Size dim(frameWidth,frameHeight);

                cv::resize(frame,frame,dim,0,0,cv::INTER_AREA);

                
                img->nx = frameWidth;
                img->ny = frameHeight;
                img->size = frameWidth * frameHeight * 3;
                img->data = new uint8_t[img->size]();

                if (frame.empty()) {
                    fprintf(stderr, "%s: failed to read frame %d from video '%s'\n", __func__, frameIndex, filename);
                    delete[] img->data;
                    return false;
                }
                ...
                fprintf(stderr, "%s: read frame %d from video '%s'\n", __func__, frameIndex, filename);

                uint8_t * frameData = frameBGR.data;
                std::memcpy(img->data, frameData, img->size);




                clip_zero_shot_label_image(ctx, n_threads, img, labels_final, num_labels, sorted_scores, sorted_indices);
                ...
                if(lastSlash != nullptr && dot != nullptr && dot > lastSlash){

                    size_t wordLength = dot - lastSlash -1;
                    char wordRecording[wordLength+1];
                    std::strncpy(wordRecording,lastSlash+1,wordLength);
                    wordRecording[wordLength] = '\0';

                    std::string filename_string = std::string(wordRecording);
                    std::string path = "../../../evaluation_results/action_recognition/" ;
                    filename_string = path + "trial_" + std::to_string(trial) + "_" + filename_string;
                    std::string output_filename = filename_string +"_"+ std::to_string(frameIndex) + ".jpg";
                    cv::imwrite(output_filename,frame);

                }

                    // Concatenate label-score pairs into a single string
                std::string csvRow =  "";

                for (int i = 0; i < num_labels; i++) {
                    auto label = labels_final[sorted_indices[i]];
                    float score = sorted_scores[i];
                    std::string labelstr(label);
                    csvRow += labelstr + " = " + std::to_string(score);
                    if (i < num_labels - 1) {
                        csvRow += ", ";
                    }
            ...
            // Calculate the elapsed time in milliseconds
                ...
                csvRow += "," + std::to_string(frameIndex) + "," + protocol + "," + oss.str() + ",";


                // Write the string to the CSV file
                file << csvRow << std::endl;


            } else {
                std::cout << "Protocol not found or result is nullptr." << std::endl;
                    // is_success_protocol = false;
    ...
    // Release the cap and close any open windows
    cap.release();
...
bool clip_zero_shot_label_image(struct clip_ctx * ctx, const int n_threads, const struct clip_image_u8 * input_img,
    ...
    for (int i = 0; i < n_labels; i++) {
        ...
        std::cout << "Text at index " << i << ": " << text << std::endl;
        clip_tokens tokens;
