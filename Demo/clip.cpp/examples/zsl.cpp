...
int main(int argc, char ** argv) {
    ...
    //load video
    const auto & vid_path = params.image_paths[0].c_str();
    if (clip_image_load_from_video(vid_path, ctx, params.n_threads, labels, n_labels, sorted_scores, sorted_indices, params.trial)) {
        fprintf(stderr, "%s: failed to load video from '%s'\n", __func__, vid_path);
        return 1;
    }
