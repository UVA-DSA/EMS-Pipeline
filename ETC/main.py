...
googlenews_model = KeyedVectors.load_word2vec_format('./word2vec/GoogleNews-vectors-negative300.bin', binary=True)
...
def lexicon_expansion(seed_terms):
    ...
    for c in seed_terms:
        if not(c.isdigit()) and (len(c) > 3):
            ...
            c_array = [s for s in c.split(' ') if s in googlenews_model]
            if len(c_array):
