...
class ExtractI3D(BaseExtractor):
    ...
    def extract(self, video_path: str) -> Dict[str, np.ndarray]:
        ...
        print("*"*20)
        print(f'Extracting features from {video_path} with {self.flow_type} flow and {self.streams} streams')
        ...
        while cap.isOpened():
            ...
            if frame_exists:
                ...
                if len(rgb_stack) - 1 == self.stack_size:
                    print("-"*20)
                    print(f'Processing stack {stack_counter} with {len(rgb_stack)} frames')
                    ...
                    print(f'Extracted stack {stack_counter} in {time.time() - start_time:.2f} seconds')
                    print("-"*20)
