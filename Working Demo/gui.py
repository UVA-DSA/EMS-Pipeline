    ...
    def generator(self):
        ...
        while not self.closed:
            ...
            if time.time() > (self.start_time + (60)):
                ...
                MsgSignal.signal.connect("API's 1 minute limit reached. Restablishing connection!")
                break
...
def GoogleSpeechFile(file):
    ...
    responses = client.streaming_recognize(streaming_config, requests)
    MsgSignal.signal.emit('Started speech recognition on audio file via Google Cloud Speech API.', 0, 0)
    num_chars_printed = 0
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        ProtcolLabel.move(1060, 70)
        ProtcolLabel.setText('<b>Protocol Modeling</b>')
        ...
        CreditsLabel.move(714, 698)
        CreditsLabel.setText('\nDesigned by Mustafa Hotaki\nand Sile Shu, May 2018\nunder Prof. Homa Alemzadeh\nfor the Dependable Systems and Analytics Lab')
    ...
    def updateSpeechBox(self, text, numPrinted, isFinal):
        ...
        if isFinal == 1:
            self.SpeechBox.setText('<html><b>' + previousTextMinusPrinted + text + ' ' + '</b></html>')
        else:
            self.SpeechBox.setText('<html><b>' + previousTextMinusPrinted + '</b></html>' + text)
    ...
    def StartGoogle(self, text, option, extra):
        previousText = self.SpeechBox.toPlainText()
        self.SpeechBox.setText('<html><b>' + previousText + '</b></html>' + '\n')
        #while(self.SpeechThread.is_alive())
        self.SpeechThread = StoppableThread(target = GoogleSpeech)
        self.SpeechThread.start()

    def updateAudioGraph(self, text, option, extra):
        pass
